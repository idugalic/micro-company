...
public class BlogPostViewEventHandler {
	...
	@EventHandler
	public void handle(final BlogPostCreatedEvent event, @SequenceNumber final Long version) {
		LOG.info("BlogPostCreatedEvent: [{}] ", event.getId());
		blogPostRepository.save(new BlogPost(event, version));
	}

	@EventHandler
	public void handle(final BlogPostPublishedEvent event, @SequenceNumber final Long version) {
		LOG.info("BlogPostCreatedEvent: [{}] ", event.getId());
		final BlogPost post = blogPostRepository.findById(event.getId()).get();
		post.setDraft(false);
